...
export interface ChatAgentStartBody {
  userId: string;
  sessionId: string;
  bookId: string;
  message: UIMessage;
...
export async function createChatStreamResponse(
  body: ChatAgentStartBody,
  options: CreateChatStreamResponseOptions,
): Promise<Response> {
  const { userId, sessionId, bookId, message, visibleText, currentChapterIndex } = body;
  ...
  await updateActiveStreamId(userId, sessionId, options.streamId);
      ...
      const result = streamText({
        ...
        tools: {
          ...
          search_book: tool({
            ...
            execute: async ({ query }) => searchBook(bookIndex, query),
          }),
          read_notes: tool({
            description:
  ...
  return createUIMessageStreamResponse({
    stream: uiStream,
    consumeSseStream: options.consumeSseStream,
  });
}

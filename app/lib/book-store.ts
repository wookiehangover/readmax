...
class BookService extends Context.Tag("BookService")<
  BookService,
  {
    ...
    readonly getBook: (
      id: string,
    ) => Effect.Effect<Book, BookNotFoundError | StorageError>;
    ...
    readonly getPosition: (
      bookId: string,
    ) => Effect.Effect<string | null, PositionError>;
  }
>() {}
  ...
  saveBook: (book: Book) =>
    Effect.tryPromise({
      try: () => set(book.id, book, bookStore),
        ...
        new StorageError({ operation: "saveBook", cause }),
    }),
  ...
  getBooks: () =>
    Effect.tryPromise({
      try: async () => {
        ...
        new StorageError({ operation: "getBooks", cause }),
    }),
    ...
    Effect.gen(function* () {
      const book = yield* Effect.tryPromise({
        try: () => get<Book>(id, bookStore),
          ...
          new StorageError({ operation: "getBook", cause }),
      });
      if (!book) {
  ...
  deleteBook: (id: string) =>
    Effect.tryPromise({
      try: () => del(id, bookStore),
        ...
        new StorageError({ operation: "deleteBook", cause }),
    }),
  ...
  savePosition: (bookId: string, cfi: string) =>
    Effect.tryPromise({
      try: () => set(bookId, cfi, positionStore),
        ...
        new PositionError({ operation: "savePosition", bookId, cause }),
    }),
  ...
  getPosition: (bookId: string) =>
    Effect.tryPromise({
      try: async () => {
        ...
        new PositionError({ operation: "getPosition", bookId, cause }),
    }),
});
